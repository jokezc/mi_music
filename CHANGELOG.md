...
- **权限请求**: 应用启动时请求必要权限（通知、网络等），提升首次使用体验。
- **设置直达**: 支持通过路由参数直接打开设置页面指定部分（如 `?section=xxx`）。
- **连接校验**: 登录/连接成功后检查当前设置与连接 host/port 是否一致，不一致时提示用户。

### 改进

- **音频状态同步**: 强制更新 MediaItem 以确保播放状态与状态栏/控制中心一致，避免内容未变化时不更新的问题。
- **音乐库**: 移除未使用的「全部歌曲」相关代码，简化 `library_page` 结构与依赖。
- **iOS 权限**: 增强局域网访问时的网络权限触发逻辑，确保能正确请求权限。
